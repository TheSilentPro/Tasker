...
public class SyncScheduler implements Scheduler {
    ...
    public SyncScheduler(JavaPlugin plugin) {
        this.plugin = plugin;
    ...
    public <T> void delayed(T value, Duration delay, Consumer<T> command) {
        scheduler.runTaskLater(plugin, () -> command.accept(value), delay.toMillis() / 50);
    }
    ...
    public void repeating(Duration initialDelay, Duration period, Runnable command) {
        scheduler.runTaskTimer(plugin, command, initialDelay.toMillis() / 50, period.toMillis() / 50);
    }
